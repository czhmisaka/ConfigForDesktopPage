...
import { btnActionTemplate, drawerProps, showType } from "@/modules/userManage/types";
import { SearchCellStorage, actionCell, staticSelectCell, tableCellTemplateMaker } from "@/modules/userManage/component/searchTable/searchTable";
...
// 批量上传
export const 批量上传按钮 = btnMaker('选择文件', btnActionTemplate.UploadFunction, {
    elType: 'success',
    icon: 'Select',
    ...
    function: async (that, da) => {
        const 开始上传 = btnMaker('开始上传', btnActionTemplate.Function, {
            ...
            function: async (th, dat) => {
                let data = da._selectedList.filter(x => (x.uploadStatus == uploadStatus.ready || x.uploadStatus == uploadStatus.error))
...
export const 删除 = btnMaker('删除', btnActionTemplate.Function, {
    icon: 'Delete',
    elType: 'danger',
    isShow: (data) => data.uploadStatus == uploadStatus.error || data.uploadStatus == uploadStatus.ready || data.uploadStatus == uploadStatus.finish,
    function: async (that, data) => {
        if (await dobuleCheckBtnMaker('删除图片', `确认删除图片【${data.name}】记录`).catch(x => false)) {
            let photos = await useCacheHook().getDataByKey(photoListKey)
            photos = photos.filter(x => x.localId != data.localId)
            useCacheHook().setup(photoListKey, async () => { return photos }, true)
...
export const 批量删除 = btnMaker('删除', btnActionTemplate.Function, {
    icon: 'Delete',
    elType: 'danger',
    isShow: (data) => data._selectedList.filter(x => {
        return x.uploadStatus == uploadStatus.error || x.uploadStatus == uploadStatus.ready || x.uploadStatus == uploadStatus.finish
    }).length > 0,
    function: async (that, data) => {
        const delPhotos = data._selectedList
        if (await dobuleCheckBtnMaker('删除图片', `确认删除图片【${delPhotos.map(x => x.name).join('】、【')}】记录`).catch(x => false)) {
            let photos = await useCacheHook().getDataByKey(photoListKey)
            delPhotos.map(del => {
...
export const 清空 = btnMaker('清空', btnActionTemplate.Function, {
    ...
    icon: 'Delete',
    isShow: (data) => !(data._selectedList.filter(x => {
        return x.uploadStatus == uploadStatus.ready || x.uploadStatus == uploadStatus.finish
    }).length > 0),
    function: async (that, data) => {
        if (await dobuleCheckBtnMaker('清空', '确认清空当前所有预备上传、已上传文件吗？').catch(x => false)) {
...
export const PictureListManage = async () => {
    const photoInfoStorage = new SearchCellStorage([
        ...
        tableCellTemplateMaker('图片地址', 'url', showCell(showType.funcComponent, {
            showFunc: (data, key) => {
                let url = data[key].replace('http://127.0.0.1:8001', window.location.origin + '/api/')
                return <ElTooltip effect="dark"
                    content={`<a href='${url}' target="_blank">${url}</a>`}
                    raw-content
                    placement="top"><ElImage style={{
                        width: '60px',
                        height: '60px'
                    }} src={url} preview-src-list={[url]} preview-teleported={true}></ElImage>
                </ElTooltip>
            }
        })),
        tableCellTemplateMaker('上传状态', 'uploadStatus', {
            ...
            ...showCell(showType.funcComponent, {
                showFunc: (data, key) => {
                    let type = 'info' as 'primary' | 'success' | 'info' | 'warning' | 'danger'
                    switch (data[key]) {
                    ...
                    return <ElTag effect="dark" type={type}>
                        {data[key]}
                    </ElTag>
                }
            })
        ...
        tableCellTemplateMaker('图片大小', 'size', showCell(showType.func, {
            showFunc: (data, key) => {
                return data[key] > 1000 * 1000 ? (data[key] / 1000 / 1000).toFixed(2) + 'mb' : (data[key] / 1000).toFixed(2) + 'kb'
            }
        })),
        tableCellTemplateMaker('标签', 'tags', showCell(showType.funcComponent, {
            showFunc: (data, key) => {
        ...
        tableCellTemplateMaker('操作', 'asd', actionCell([删除], {
            fixed: "right",
        }))
    ])
    ...
    return [
        gridCellMaker('searchTable', 'searchTable', {}, {
            ...
            props: {
                ...
                searchFunc: async (query, that) => await useCacheHook().getDataByKey(photoListKey),
                defaultQuery: {
                    showLink: null,
                },
